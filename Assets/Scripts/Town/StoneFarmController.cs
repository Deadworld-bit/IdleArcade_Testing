...
public class StoneFarmController : MonoBehaviour
{
    ...
    {
        float availableSpace = townController.townStoneMaximum - townController.townStone;
        if (currentResources <= availableSpace)
        {
            townController.townStone += currentResources;
            currentResources = 0;
        }
        else
        {
            townController.townStone += availableSpace;
    ...
    public void LevelUp()
    {
        if (townController.townGold >= (structureLevel * 1000) && townController.townWood >= (structureLevel * 1000) && structureLevel < 3)
        {
            townController.townGold -= structureLevel * 1000;
            townController.townWood -= structureLevel * 1000;
            structureLevel++;
